...
def connect_api_key():
    api_key="AIzaSyDtQ4TiR_ndy8Jwcx1A2Fe21KYVJSKUWFQ"
...
# Home_page settings
if web=="HOME":
    slt.image("youtube-logo-8.png",width=200)
    slt.title("YouTube Data Harvesting and Warehousing using SQL and Streamlit")
    slt.subheader(":red[Domain:]  Social Media")
    slt.subheader(":red[Overview:]") 
    slt.markdown('''Build a simple dashboard or UI using Streamlit and 
    ...
    slt.markdown("Python scripting,Data Collection,API integration,Data Management using SQL,Streamlit")
    slt.subheader(":red[Developed-by:]  Kavitha Thangavel")
...
if web=="DATA COLLECTION":
    ...
    if slt.button("submit"):
        slt.success("Link submitted successfully")
...
if web=="MIGRATE TO SQL":
    ...
    if slt.button("migrate to sql"):
        conn = mysql.connector.connect(host="localhost", user="root", password="kavi",database="youtube")
        ...
        for index, row in df_channel.iterrows():
            table_insert_query = '''INSERT INTO channel (channel_name, channel_id, subscriptioncount, viewscount, description, playlist_id)
                                    VALUES (%s, %s, %s, %s, %s, %s)'''
            values = (row["channel_name"], row["channel_id"], row["subscriptioncount"], row["viewscount"], row["description"], row["playlist_id"])
        ...
        for index, row in df_video.iterrows():
            ...
            values = (row["Video_id"], row["video_name"],row["channel_name"], row["video_description"], row["PublishedAt"], row["view_count"], row["like_count"],
        ...
        conn.commit()
        my_cursor.close()
        slt.success("MIGRATED SUCCESSFULL")
